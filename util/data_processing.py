...
def split_and_normalize(_df, season_max_length, seasons, features, ferguson_features, label, phenos_list = [], x_mean=None, x_std=None, ferguson_mean=None, ferguson_std=None, experiment=None):
    x = []
    y = []
    ferguson = []
    for i, season in enumerate(seasons):
        ...
        for p in phenos_list:

            _phen = np.zeros((season_max_length, 1))
            _phen[:] = np.NaN

            season_df = _df["PHENOLOGY"].iloc[season[0]:season[-1]]

            pheno_index = season_df.loc[season_df == p].index.values

            if (len(pheno_index) == 1):
                pheno_index = pheno_index[-1] - season[0]
                #print("budbreak idx:",  budbreak_index)
                #print("_y before", list(_y.flatten()))
                
                if experiment in ['encode_cultivar','encode_cultivar_2']:
                    if False:
                        pheno_index = np.random.randint(200,252)
                    #_phen[:] = pheno_index/252 #normalize
                    _phen[:] = (pheno_index-225)/25 #normalize
                else:
                    _phen[0:pheno_index] = 0
                    _phen[pheno_index:] = 1
...
def data_processing_multiple_cultivars(cultivar_file, cultivar_idx, args):
    ...
    pheno_seasons = 0
    lte_pheno_seasons = 0
    lte_pheno_datapoints = 0
    '''
    for season, dormant_season in zip(seasons, dormant_seasons):
        ...
        valid_idx = list(np.array(dormant_season)[~np.isnan(df['PREDICTED_LTE50'].iloc[dormant_season].to_numpy())])
        valid_lte_readings = list(np.array(dormant_season)[~np.isnan(df['LTE50'].iloc[dormant_season].to_numpy())])

        dfsubset = df.PHENOLOGY.iloc[season]
        valid_pheno = 0
        for pheno in args.phenos_list:
            valid_pheno += dfsubset[dfsubset==pheno].shape[0]
        ...
        if (len(valid_idx) >= int(0.9*len(dormant_season))) and (missing_temp <= int(0.1*len(season))) and (len(valid_lte_readings)>0) and (valid_pheno==len(args.phenos_list)):
            valid_seasons.append(season)
        #TODO remove!!! 
    '''        
        if (len(valid_idx) >= int(0.9*len(dormant_season))) and (missing_temp <= int(0.1*len(season))):
            if (len(valid_lte_readings)>0) and (valid_pheno==len(args.phenos_list)):
                lte_pheno_seasons += 1
                lte_pheno_datapoints += len(valid_lte_readings)
            if (len(valid_lte_readings)>0):
                lte_seasons += 1
                lte_datapoints += len(valid_lte_readings)
            if (valid_pheno==len(args.phenos_list)):
                pheno_seasons += 1
            
    print("lte: ", lte_seasons, lte_datapoints, "pheno: ", pheno_seasons, "both: ", lte_pheno_seasons, lte_pheno_datapoints)
    '''
    ...
    # if cultivar_file=='Riesling' and args.no_seasons!=-1:
    ...
    x_train, y_train, ferguson_train = split_and_normalize(
        df, season_max_length, train_seasons, args.features, args.ferguson_features, args.label, args.phenos_list, x_mean, x_std, ferguson_mean, ferguson_std, args.experiment)

    x_test, y_test, ferguson_test = split_and_normalize(
        df, season_max_length, test_seasons, args.features, args.ferguson_features, args.label, args.phenos_list, x_mean, x_std, ferguson_mean, ferguson_std, args.experiment)
